...
const GuideSection = ({ title, icon, items, basePath }: GuideSectionProps) => (
  <div className="mb-12">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.map((item) => (
        <Link href={`${basePath}/${item.id}`} key={item.id} className="group h-full">
          <div className="h-full bg-white rounded-xl p-6 shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-md hover:border-[#5b8064]/50 hover:-translate-y-1 flex flex-col">
...
export default async function GuidePage() {
  const lifeFiles = (await getLifeFiles()).filter(item => item.type !== 'guideline');
  const economyFiles = (await getEconomyFiles()).filter(item => item.type !== 'guideline');
  const adventureFiles = (await getAdventureFiles()).filter(item => item.type !== 'guideline');
  const transportFiles = (await getTransportFiles()).filter(item => item.type !== 'guideline');
          ...
          <div className="mt-20 text-center">
            <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  href="/tutorial"
                  className="inline-flex items-center justify-center px-6 py-3 bg-gray-900 text-white rounded-xl hover:bg-gray-800 font-medium transition-colors shadow-lg shadow-gray-900/20"
                >
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center px-6 py-3 bg-[#5865F2] text-white rounded-xl hover:bg-[#4752C4] font-medium transition-colors shadow-lg shadow-[#5865F2]/20"
                >
